...
if av is None:
    ...
    if parent_view is None:
        ...
        if switched:
            ...
            # Force Revit to actually repaint the newly-active view RIGHT
            # NOW, synchronously, instead of waiting for it to get around
            # to it after this command returns. This is what makes the
            # zoom below reliable -- no event subscriptions needed at all
            # (an earlier version of this script used a one-shot Idling
            # handler for this instead, which is what caused the Filter
            # dialog / other Revit dialogs to stop responding: any handler
            # left attached to an application-wide event, even briefly,
            # risks interfering with unrelated UI elsewhere in Revit).
            try:
